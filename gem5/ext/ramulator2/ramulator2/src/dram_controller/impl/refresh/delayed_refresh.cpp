...
class DelayedRefresh : public IRefreshManager, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IRefreshManager, DelayedRefresh, "DelayedRefresh", "Delays refreshes and sends them together.")
  private:
    Clk_t m_clk = 0;
    IDRAM* m_dram;
  ...
  public:
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      m_max_delays =  param<int>("max_delays").desc("Maximum number of intervals without refresh.").default_val(1);
      m_next_refresh_cycle = m_max_delays * m_nrefi;
    };
    ...
    void tick() {
      ...
      if (m_clk == m_next_refresh_cycle) {
        ...
        for (int r = 0; r < m_num_ranks; r++) {
          ...
          for (int num_ref = 0; num_ref < m_max_delays; num_ref++) {
