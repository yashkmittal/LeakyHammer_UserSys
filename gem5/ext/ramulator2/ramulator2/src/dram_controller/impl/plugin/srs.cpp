...
class SRS : public IControllerPlugin, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IControllerPlugin, SRS, "SRS", "Secure Row-Swap.")
  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
      ...
      if (m_clk % m_reset_period_clk == 0) {
        ...
        if (m_is_debug) {
          std::cout << "----------------------------" << std::endl;
          std::cout << "SRS is resetting. " << m_clk << std::endl;
      ...
      if (request_found) {
        if (m_dram->m_command_meta(req_it->command).is_opening && m_dram->m_command_scopes(req_it->command) == m_row_level) {
          ...
          if (m_is_debug) {
            std::cout << "----------------------------" << std::endl;
            std::cout << "SRS: ACT on row " << row_id << "         " << m_clk << std::endl;
